...
ECOTree._firstWalk = function (tree, node, level) {
        ...
        if(node._getChildrenCount() == 0 || level == tree.config.iMaxDepth)
        {
            ...
            if(leftSibling != null)
                node.prelim = leftSibling.prelim + tree._getNodeSize(leftSibling) + node.config.iSiblingSeparation;
            else
                node.prelim = 0;
        } 
        else
        {
            ...
            for(i in node.childsId)
            {
                ...
                // ECOTree._firstWalk(tree, node.tree.nodos[node.childsId[i]], node.Level);
				// node.Level=level + 1;
            }
...
ECOTree._apportion = function (tree, node, level) {
        ...
        for(var k = tree.config.iMaxDepth - level; firstChild != null && firstChildLeftNeighbor != null && j <= k;)
        {
			...
			/// Adicionado por el algoritmo de dibujo de bolitas en vez de cuadros
			if(!node.bolita)
				var variable = tree.config.iSubtreeSeparation;
			else
				var variable = 20;
			///
            var totalGap = (firstChildLeftNeighbor.prelim + modifierSumLeft + tree._getNodeSize(firstChildLeftNeighbor) + variable) - (firstChild.prelim + modifierSumRight);
            if(totalGap > 0)
            {
                var subtreeAux = node;
                var numSubtrees = 0;
