...
ECONode = function (id, pid, dsc, w, h, c, bc, target, meta) {
	...
	* Param {String} dsc, descripcion del nodo
	* Param {Number} w, define el ancho del nodo
	* Param {Number} h, define el alto del nodo
	...
	if (pid == -1) 
	...
	else
		{
			for (var k = 0; k < this.nDatabaseNodes.length; k++)
			{
				...
				{
					pnode = this.nDatabaseNodes[k];				//buscando el objeto padre, localizado
					break;
				}
			}	
		}
	
	var node = new ECONode(id, pid, dsc, nw, nh, color, border, tg, metadata);	//crear el nodo nuevo
