...
Components.curvaS.prototype.init = function(dataObj) {
	console.log('init curvas');
this.config = {
		container: $('body'),
		id: "curvaS-"+ Math.round(Math.random() * 2000),
		width: -1,
		height:-1,
		css:new Array(),		
		hidden: false,
	...
	//svgcontainer enviara estos datos--------------------------------------
	this.root=dataObj.container;
	this.svgContend = dataObj.svgcontainer.svgContend;
	//--------------------------------------------------------------------
	this.svg = this.config.svgcontainer.svg;	//lienzo svg creado por svg container, componente padre obligatorio de todo componente que dibuje cosas SVG
	this.id=this.config.id;
	this.datos = this.config.datos;
	//----> Fritz  estos valores se calculan del svgcontainer
	this.anchoBase = 600;
	this.altoBase = 400;
	
	
	//this.svg = data["svg"];
	this.tipo = dataObj["tipo"];
	var inicio = dataObj["padre"];
	this.padre = dataObj["padre"];
	...
	this.origenX = this.config.origenX;							//margen a la izquierda para iniciar el diagrama
	this.escalaX = this.config.escalaX;
	this.escalaY = this.config.escalaY;
	this.MarginTopDC = this.config.MarginTopDC;
	this.MarginBottomDC = this.config.MarginBottomDC;
	this.MarginLeftDC = this.config.MarginLeftDC;
	this.MarginRightDC = this.config.MarginRightDC;
	this.zoomControl = {
		enable:false,
		xs:0,
		xf:0,
	...
	this.iniFec = dataObj['iniFec'];		//fecha de inicio del proyecto, formato timestamp	
	this.finFec = dataObj['finFec'];		//fecha de fin del proyecto, formato timestamp
	...
	this.BarrasNombres = dataObj["barrasnombres"];///
	this.BarrasValores= dataObj["barrasvalores"];//	
	this.g = 0;
	this.defs = 0;
 ...
 //##########################################################################################################################
 Components.curvaS.prototype.create = function(){
	 this.draw();
 }
 //##########################################################################################################################
Components.curvaS.prototype.draw = function() {
...
//#################################################################################
Components.curvaS.prototype.ZoomOf = function(points) {
...
//#################################################################################
//#################################################################################
Components.curvaS.prototype.calcularHitos = function() {
...
//#################################################################################
Components.curvaS.prototype.linealizeData = function() {
...
//#################################################################################
Components.curvaS.prototype.IncZoomX = function() {
	this.escalaX += 0.5;
	this.redraw();
}
//#################################################################################
Components.curvaS.prototype.DecZoomX = function() {
	this.escalaX -= 0.5;
	this.redraw();
}
//#################################################################################
Components.curvaS.prototype.IncZoomY = function() {
	this.escalaY = this.escalaY + 0.5;		
	this.redraw();
}
//#################################################################################
Components.curvaS.prototype.DecZoomY = function() {
	this.escalaY = this.escalaY - 0.5;
	this.redraw();
}
//#################################################################################
Components.curvaS.prototype.redraw = function() {
	this.removeGraphics();
	this.draw();
}
//#################################################################################
Components.curvaS.prototype.resetScale = function() {
	this.escalaX = 1;
	this.escalaY = 1;
	this.redraw();
}
Components.curvaS.prototype.removeGraphics = function() {
	this.svg.clear();
	this.points = new Array();
	this.curva[0] = null;
	this.hitos = new Array();
...
//#################################################################################
Components.curvaS.prototype.drawInfo = function() {
	this.info = new Array();
if(!this.activo){	
...
//#################################################################################
Components.curvaS.prototype.generarDatoAcumulativo = function(rango,dataset) {
...
//#################################################################################
Components.curvaS.prototype.drawCurva = function() {
...
//--------------------------------------------------------------------------------------------------------------------------------
Components.curvaS.prototype.drawBarras = function(dataset) {
...
//#################################################################################
Components.curvaS.prototype.drawAxisX = function(configX) {
var dist;
...
//#################################################################################
Components.curvaS.prototype.drawAxisY = function() {
...
//#################################################################################
Components.curvaS.prototype.showAllHitos = function() {
...
//#################################################################################
Components.curvaS.prototype.showHito = function(hitoId) {
...
//#################################################################################
//#################################################################################
Components.curvaS.prototype.hideAllHitos = function() {
...
//#################################################################################
Components.curvaS.prototype.hideHito = function(hitoId) {
...
//#################################################################################
Components.curvaS.prototype.mayor = function() {
var mayor=0;
...
//#################################################################################
Components.curvaS.prototype.ActivarZoom = function() {
	...
	console.log(this.svgContend);
	this.svgContend.bind('mousedown', {obj: this}, function(evt) {
		points["x1"] = evt.offsetX;
		points["y1"] = evt.offsetY;
		var diagrama = evt.data.obj;
		diagrama.selector.setAttribute("visibility", "visible");
		diagrama.selector.setAttribute("x", points.x1);
		diagrama.selector.setAttribute("y", points.y1);
		
		diagrama.svgContend.bind('mousemove', {diagram: diagrama}, function(e) {
			console.log('mousemove');
			var difx = Math.abs(points.x1 - e.offsetX);
			var dify = Math.abs(points.y1 - e.offsetY);
			
			e.data.diagram.selector.setAttribute("width", difx);
			e.data.diagram.selector.setAttribute("height", dify);
		});
	});
	
	this.svgContend.bind('mouseup', {obj: this}, function(evt) {
...
Components.curvaS.prototype.DesactivarZoom = function() {
    this.svgContend.unbind('mousedown');
    this.svgContend.unbind('mouseup');
}
...
//-------------------------------------------------------------------------------------------
Components.curvaS.prototype.resizeBody = function(){
//calculamos el elemento mas a la izquierda y el que esta mas a la derecha.
//----> Fritz  estos valores se calculan
var ancho = 1200
var altura = 600;
